...
class Game {
  constructor() {
    ...
    const PostData = Moralis.Object.extend("UserPosts");
    const query = new Moralis.Query(PostData);
    query.select("saveData");
    const post = query.find();
    let abc = Promise.all([post]).then((val) => {
      const data = JSON.parse(JSON.stringify(val));
      console.log(data[0]);
      data[0].map(element => {
        console.log(element,"element");
        this.imagesMarket.push(element.saveData.postImage);
        this.imagesStadium.push(element.saveData.postImage);
        this.imagesPost.push(element.saveData.postImage);
      });
  ...
  loadCubePost(gltf, game) {
    ...
    for (let i = 0; i < 4; i++) {
      const loader = new THREE.TextureLoader();
      const mapOverlay = loader.load(game.imagesPost[i]);
      let dice = new THREE.Mesh(
        new THREE.BoxGeometry(200, 500, 200, 1, 1, 1),
